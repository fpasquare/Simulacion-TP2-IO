"""
Simulación de Estación de Servicio – Red Río Seco
11.51 Investigación Operativa – TP2a – Grupo 6
...
Modelo conceptual implementado:
  ...
  · 3.4% de vehículos eléctricos (cargador 50 kW → 48 min)
...
FRAC_EV         = 0.034           # fracción del total (3.4 %)
TIEMPO_CARGA_EV = 48.0            # min (cargador 50 kW, ~40 kWh promedio)
...
class Vehiculo:
    ...
    def __init__(self, vid: int, t_llegada: float, hay_inexperto: bool = False):
        ...
        self.es_ev       = random.random() < FRAC_EV
        self.tipo        = self.asignar_tipo(self.PROB_TIPO)
        self.combustible = "electrico" if self.es_ev else self.asignar_tipo(self.PROB_COMB)
...
if __name__ == "__main__":
    ...
    print(f"  EV: {FRAC_EV*100:.1f}%  |  Cargador EV: {TIEMPO_CARGA_EV:.0f} min  "
          f"|  Corridas: {NUM_CORRIDAS}")
